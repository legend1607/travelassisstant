(function installTripMapState(root) {
  ...
  function baseAssignments(trip) {
    ...
    for (const day of trip.days || []) {
      for (const stop of day.routeStops || []) {
        if (Object.prototype.hasOwnProperty.call(assignments, stop.poiId)) {
  ...
  function baseOrders(trip, assignments) {
    ...
    for (const day of trip.days || []) {
      const stops = [...(day.routeStops || [])]
        .filter((stop) => assignments[stop.poiId] === day.id && pois.has(stop.poiId))
  ...
  function normalizeOrders(trip, savedOrders, assignments) {
    ...
    for (const day of trip.days || []) {
      const requested = Array.isArray(savedOrders?.[day.id]) ? savedOrders[day.id] : [];
      const valid = requested.filter(
        (poiId, index) =>
          pois.has(poiId) &&
          assignments[poiId] === day.id &&
      ...
      valid.sort((leftId, rightId) => {
        const leftHotel = pois.get(leftId)?.category === "hotel";
        const rightHotel = pois.get(rightId)?.category === "hotel";
        if (leftHotel === rightHotel) return 0;
        return leftHotel ? -1 : 1;
      });
      orders[day.id] = valid;
    }
    return orders;
  ...
  function normalizeState(trip, saved, includeUndo) {
    ...
    for (const poi of trip.pois || []) {
      ...
      assignments[poi.id] = requestedDay === "" || days.has(requestedDay) ? requestedDay : defaults[poi.id];
  ...
  function activeNonHotelCount(trip, state, dayId) {
    const pois = poiMap(trip);
    return Object.entries(state.assignments || {}).filter(
      ([poiId, assigned]) => assigned === dayId && pois.get(poiId)?.category !== "hotel",
    ).length;
  }
