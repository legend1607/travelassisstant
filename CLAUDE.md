...
法则: 行程是参考坐标，不是执行脚本。AMap 是唯一地图引擎。env.js 已 gitignore，使用 env.example.js 作为模板。大众点评和小红书需要已登录的浏览器会话，headless 不可用。记忆只存下次仍有用的偏好。餐厅先看当天区域，再看大众点评和小红书。大众点评注意刷好评，小红书注意推广帖和差评。
